...
/// <summary>
/// Implementação do serviço de integração com PHC WEB para o módulo Advances.
/// Executa o script insertRdAPI via SOAP (RunCode) para criação de adiantamentos.
/// </summary>
public class PhcWebService : IPhcWebServiceAdvances
{
    ...
    /// <inheritdoc />
    public async Task<string> CreateAdvanceAsync(string parameter, CancellationToken ct = default)
    {
        var code = _configuration["PhcWeb:Scripts:InsertRd:Code"]
            ?? throw new InvalidOperationException("PhcWeb:Scripts:InsertRd:Code não configurado");

        try
        {
            _logger.LogInformation("Starting PHC WEB integration for Advances script: {Code}", code);
