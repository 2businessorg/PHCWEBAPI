...
When cloud is on, `AnalyzeCandidateJob` calls `IRecruitmentCloudEgressGuard` then `QwenCloudScoreEngine` through `IRecruitmentLlmClient` → existing `ILocalChatModel` (DashScope compatible-mode). The prompt is `qwen-cloud-v2`. Quotes must be exact spans of the **pseudonymized** text. `u_modeloia` stores the model name (`LocalAI:Model`, e.g. `qwen3-coder-next`) and `u_prmveria` stores `qwen-cloud-v2`. `used_llm` in `u_justia` is true.
