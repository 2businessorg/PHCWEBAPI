...
- Engine: Qwen cloud when `RecruitmentIa:EnableCloudLlm=true` (`qwen-cloud-v4`, `ILocalChatModel`). Assisted decision is only `shortlist_suggest`, `interview_suggest`, `weak_fit_suggest`, `insufficient_evidence`, or `conflict_review`. Scorecard fields for the later report live in `u_justia`; Presidio audit lives in `u_auditia`. Offline rubric `rubric-evidence-v1` only when the flag is false. Embeddings REJECTED as sole engine.
