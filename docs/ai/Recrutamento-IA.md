...
- Engine: Qwen cloud when `RecruitmentIa:EnableCloudLlm=true` (`qwen-cloud-v7`, `ILocalChatModel`). Assisted decision is only `avancar`, `em_duvida`, or `nao_avancar`. Scorecard fields for the later report live in `u_justia`; Presidio audit lives in `u_auditia`. Offline rubric `rubric-evidence-v1` only when the flag is false. Embeddings REJECTED as sole engine.
