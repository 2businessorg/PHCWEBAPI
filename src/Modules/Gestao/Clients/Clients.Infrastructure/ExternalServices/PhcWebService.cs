...
namespace Clients.Infrastructure.ExternalServices;

/// <summary>
/// Implementacao do servico de integracao com PHC WEB para o modulo Clients.
/// </summary>
public class PhcWebService : IPhcWebServiceClients
{
    ...
    /// <inheritdoc />
    public async Task<string> CreateClientAsync(string parameter, CancellationToken ct = default)
    {
        var code = _configuration["PhcWeb:Scripts:InsertCl:Code"]
            ?? throw new InvalidOperationException("PhcWeb:Scripts:InsertCl:Code nao configurado");

        try
        {
            _logger.LogInformation("Starting PHC WEB integration for Clients script: {Code}", code);
        ...
        {
            _logger.LogError(ex, "Error executing PHC WEB Clients script {Code}: {Message}", code, ex.Message);
            throw;
        }
    }
    ...
    {
                var escapedJson = SecurityElement.Escape(parameter);
