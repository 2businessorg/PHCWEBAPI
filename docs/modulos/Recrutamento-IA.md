...
`recommendation.decision` is only `shortlist_suggest`, `interview_suggest`, `weak_fit_suggest`, `insufficient_evidence`, or `conflict_review`. AH-05 conflict maps to `conflict_review` (quotes in `conflicts[]`). A criterion without evidence maps to `insufficient_evidence` and fills `gapsPt`. All evidenced and score ≥ 85% of the RCT weight maps to `shortlist_suggest`; ≥ 70% maps to `interview_suggest`; below that maps to `weak_fit_suggest`. `labelPt` always contains `Sugestão` and `decisão humana obrigatória`. `disclaimerPt` states that the score is input and is not selecção, rejeição, or avanço. `humanDecisionRequired` and `scoreIsInputNotDecision` are forced true on save. `criteria.status` is `evidenced`, `no_evidence`, or `conflict`, with `weightSource` `rct`. Tokens such as `selected`, `rejected`, `advanced`, `hired`, `approved`, `auto_*`, `pass`, and `fail` are not stored.
